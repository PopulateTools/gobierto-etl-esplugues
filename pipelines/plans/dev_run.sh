...
cd $DEV/gobierto-etl-esplugues/; ruby operations/gobierto_plans/extractor/run.rb $WORKING_DIR/plan.json

# Extract > Check JSON format
cd $DEV/gobierto-etl-utils/; ruby operations/check-json/run.rb $WORKING_DIR/plan.json

# Load > Import plans
cd $DEV/gobierto; bin/rails runner $DEV/gobierto-etl-esplugues/operations/gobierto_plans/importer/run.rb $WORKING_DIR/plan.json esplugues.gobify.net pam-2016-2019 reset_previous_data
